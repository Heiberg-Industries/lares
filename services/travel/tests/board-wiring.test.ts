...
// BOTH directories, since ORB-278 step 2 (ADR-0015 rule 3): Marcel's real tools live in the
// `catalogue/` pool now, and what is left under agent/tools/ is the nine `disableTool()`
// sentinels, the one web_search framework-tool enable, and the one resolver. Reading only
// agent/tools/ would have left every moved tool unchecked and every assertion below vacuously
// green.
