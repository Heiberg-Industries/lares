...
describe("the catalogue index", () => {
  ...
  it("never carries web_search — Marcel's one framework-tool enable is engine policy, not a grant", () => {
    // agent/tools/web_search.ts is NOT a disableTool() sentinel: it is the authored re-enable of
    // eve's own built-in `web_search`, wired through `framework_tools` in agent.json, not through
    // a capability grant. It has no place in a POOL the definition picks capabilities from — the
    // task brief that added this catalogue calls that out explicitly, and this is the pin.
    expect(CATALOGUE["web_search"]).toBeUndefined();
  });
});
...
const GRANTED_AREAS: readonly string[] = ["facts"];
const VAULT_TOOLS_BEFORE = ["remember"];
