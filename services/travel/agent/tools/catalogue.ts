...
//
// THE APPROVAL IS COPIED ACROSS, AND THAT LINE IS LOAD-BEARING — for the general case. None of
// Marcel's twenty-one own tools carry an `approval` (he has no HITL tool surface at all — his
// Telegram approval cards live in schedules, not in tools; `tests/agent-declaration.test.ts`
// pins that), and `agent-kit__transit_plan` carries none either (a free, unbilled public read).
// So for THIS service the line below never actually copies anything today — but it stays,
// unconditional, because the day a gated tool joins this catalogue is the day this resolver must
// already be correct, not the day someone remembers to add the line. Task 1 measured that a
...
// so omitting this would ship a gated write with no card the day one exists.
