...
See `config.example` for build-time settings. Keep the actual public project token in build configuration; private PostHog management credentials never enter this app. Localhost and preview hosts never send analytics, even with consent. The SDK loads only after consent on lares.is or www.lares.is. Replay, automatic capture and user identification are off. A strict property allowlist drops arbitrary fields and URL queries.
