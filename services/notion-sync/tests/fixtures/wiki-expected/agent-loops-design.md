...
<callout icon="💡" color="blue">
	Claire Vo's practical how-to on **loop design**: a loop is "just an automated<br>prompt." Four ways to fire one (heartbeat, cron, hook, goal), five things every<br>loop needs before production, and an "onboard an employee" mental model — shown<br>through two live builds in Claude Code and Codex.
</callout>
...
<callout icon="⚠️" color="yellow">
	Two red flags that a loop will burn money: **inefficient goal-based structures**<br>(recursive loops that don't converge) and **unnecessary token-consumption<br>patterns**. Token cost is the recurring caveat across both this and<br>\[\[loop-engineering-designing-loops-instead-of-prompting-agents\]\] — usage varies<br>wildly depending on whether you're token-rich or token-poor.
</callout>
...
- <mention-page url="https://www.notion.so/44444444444444444444444444444444">Services as Software — copilots become autopilots</mention-page> — loops are how "autopilots" actually run unattended
- <mention-page url="https://www.notion.so/22222222222222222222222222222222">A Camera, Not an Engine — seeing in latent space, and the camera/engine split in agents</mention-page> — the theory under the loop: error signal = feedback step; camera (explore) vs engine (exploit) modes
