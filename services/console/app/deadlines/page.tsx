...
export default async function DeadlinesPage() {
  ...
  const mintRules: MintRule[] = STATUTORY_RULES_MIRROR.map((r) => ({
    key: r.key,
    title: r.title,
    standingDate: standingDateLabel(r.month, r.day),
  ...
  const candidates: CandidateDTO[] = view.candidates.map((c) => ({
    ...
    subject: c.subject,
    sender: c.sender,
    seenAtLabel: c.seenAt.toISOString().slice(0, 10),
    surfaced: c.surfacedAt !== null,
  }));
    ...
    <div className="lares-page lares-operational">
      ...
      <table className="card" style={{ marginTop: 8 }}>
        ...
        <tbody>
          {openRows.length === 0 && (
            <tr>
              <td colSpan={9} style={{ color: "var(--mist)" }}>
                Nothing open.
              </td>
            </tr>
          )}
          {openRows.map((row) => (
            <OpenRow key={row.id} row={row} />
          ))}
        </tbody>
      </table>
      ...
      <table className="card" style={{ marginTop: 8 }}>
        ...
        <tbody>
          {candidates.length === 0 && (
            <tr>
              <td colSpan={5} style={{ color: "var(--mist)" }}>
                No unresolved candidates.
              </td>
            </tr>
          )}
          {candidates.map((c) => (
            <CandidateRow key={c.threadId} candidate={c} />
          ))}
        </tbody>
      </table>
      ...
      <table className="card" style={{ marginTop: 8 }}>
        ...
        <tbody>
          {view.closed.length === 0 && (
            <tr>
              <td colSpan={6} style={{ color: "var(--mist)" }}>
                Nothing closed yet.
              </td>
            </tr>
          )}
          {view.closed.map((d) => (
            <tr key={d.id}>
              ...
              <td
                ...
                style={{
                  fontSize: 12,
                  color: d.status === "done" ? "var(--ok)" : "var(--mist)",
                }}
              >
                {d.status}
              </td>
