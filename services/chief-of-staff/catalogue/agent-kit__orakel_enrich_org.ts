...
// through `agent/extensions/agent-kit/tools/orakel_enrich_org.ts` (ORB-278 step 2, Task 9). That
// override file is now an unconditional `disableTool()` sentinel — see its own header — because a
// resolver-emitted tool under the same prefixed key (`agent-kit__orakel_enrich_org`) replaces an
...
// it (packages/agent-kit/extension/tools/orakel_enrich_org.ts).
import { orakel_enrich_org } from "@lares/agent-kit/tools";

export default orakel_enrich_org;
