...
// through `agent/extensions/agent-kit/tools/orakel_search.ts` (ORB-278 step 2, Task 9). That
// override file is now an unconditional `disableTool()` sentinel — see its own header — because a
// resolver-emitted tool under the same prefixed key (`agent-kit__orakel_search`) replaces an
...
// it (packages/agent-kit/extension/tools/orakel_search.ts).
import { orakel_search } from "@lares/agent-kit/tools";

export default orakel_search;
