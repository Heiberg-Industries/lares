-- FIXTURE for tests/eve-workflow-upgrade.test.ts (W2-s8c). Frozen copy of
-- services/chief-of-staff/sql/001-eve-workflow.sql exactly as committed before it was
-- regenerated for @workflow/world-postgres beta.42 (the pre-regeneration, 33-statement,
-- beta.32 shape — old primary key `workflow_events_pkey` on (id) alone). Stands in for "an
-- existing installation's database" in the proof that 006-eve-workflow-beta32-to-beta42-upgrade.sql
-- carries real rows from this exact shape to the regenerated one. Never applied outside a test;
-- never hand-edited to track a real box.
--
-- eve workflow role: @workflow/world-postgres@5.0.0-beta.32 schema (extracted for manual box deploy)
--
...
-- equivalent, meant to be applied once by hand before the box's eve workflow role container starts.
--
...
-- concurrent DDL. Not a concern for a single eve workflow role instance; worth another look if the box
-- ever runs eve workflow role as more than one replica.)
--
...
DO $$ BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'workflow_events_pkey' AND connamespace = 'workflow'::regnamespace
    ) THEN
...
CREATE INDEX IF NOT EXISTS workflow_events_run_id_index ON workflow.workflow_events USING btree (run_id);
