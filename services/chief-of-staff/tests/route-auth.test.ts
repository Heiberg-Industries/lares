...
function basic(user: string, pass: string): Request {
  ...
  return new Request("https://eve-saga.test/eve/v1/session", {
    method: "POST",
    headers: { authorization: `Basic ${encoded}` },
  });
}

describe("eve-saga route auth", () => {
  let dir: string;
  let pwPath: string;

  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), "eve-saga-routeauth-"));
    pwPath = join(dir, "route-password");
    writeFileSync(pwPath, "s3cret-route-password\n", "utf8");
    process.env["EVE_SAGA_ROUTE_PASSWORD_FILE"] = pwPath;
  });

  afterEach(() => {
    rmSync(dir, { recursive: true, force: true });
    delete process.env["EVE_SAGA_ROUTE_PASSWORD_FILE"];
  });

  it("accepts the correct credential (trailing newline in the secret file is trimmed)", async () => {
    const auth = await freshAuth();
    const result = await auth(basic("eve-saga", "s3cret-route-password"));
    expect(result).toBeTruthy();
  });

  it("rejects a wrong password", async () => {
    const auth = await freshAuth();
    expect(await auth(basic("eve-saga", "wrong"))).toBeFalsy();
  });
  ...
  it("rejects a request with no Authorization header — this is the production default", async () => {
    const auth = await freshAuth();
    const bare = new Request("https://eve-saga.test/eve/v1/session", { method: "POST" });
  ...
  it("throws on an empty secret file rather than authenticating an empty password", async () => {
    ...
    const auth = await freshAuth();
    await expect(async () => auth(basic("eve-saga", ""))).rejects.toThrow(/secret file is empty/);
  });

  it("throws a path-only error when the secret file is missing", async () => {
    process.env["EVE_SAGA_ROUTE_PASSWORD_FILE"] = join(dir, "does-not-exist");
    const auth = await freshAuth();
    await expect(async () => auth(basic("eve-saga", "whatever"))).rejects.toThrow(
