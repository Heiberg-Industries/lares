...
describe("the catalogue index", () => {
  ...
  it("never carries commercial_who_to_contact — it is skill-gated, not capability-gated", () => {
    // resolveSkillTool (agent/tools/commercial_who_to_contact.ts, matching agent/extensions/
    // agent-kit/tools/market_edge.ts's own seam) checks a SKILL declaration, not a capability
    // grant. grantedToolNames only ever asks a capability's grant, so putting this tool in the
    // pool would silently swap what gates it. See this file's own header, catalogue/index.ts's.
    expect(CATALOGUE["commercial_who_to_contact"]).toBeUndefined();
  });

  it("carries agent-kit__market_edge, gated on the market-edge skill (LAR-5-s2, Task 10)", () => {
...
const GRANTED_AREAS: readonly string[] = ["private", "shared", "facts"];
const VAULT_TOOLS_BEFORE = ["agent-kit__vault_backlinks", "agent-kit__vault_drop", "agent-kit__vault_file", "agent-kit__vault_list", "agent-kit__vault_read", "agent-kit__vault_search", "agent-kit__vault_write", "atlas_proposals", "atlas_resolve_proposal", "facts_list", "forget", "memory_proposals", "memory_resolve_proposal", "memory_used", "remember", "save_note"];
