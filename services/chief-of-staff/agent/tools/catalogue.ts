...
//
// THE APPROVAL IS COPIED ACROSS, AND THAT LINE IS LOAD-BEARING — here more than anywhere else in
// the fleet so far. `agent-kit__vault_write`, `agent-kit__vault_file` and `agent-kit__vault_drop`
// each carry `approval: always()` (a vault write or delete) — Task 1 measured that a
// resolver-emitted tool REPLACES an authored/mounted tool of the same name completely —
// description, executor AND approval (docs/research/2026-09-16-eve-0.32-dynamic-seams.md, Q1) —
// so omitting this line would ship those three gated writes with no card. The other sixty-four
// entries in `../../catalogue/index.ts` carry no approval today, so for them this line copies
// nothing; it stays unconditional because the day another gated tool joins this catalogue is the
// day this resolver must already be correct, not the day someone remembers to add the line.
...
// LAR-5-s2: `agent-kit__market_edge` moved into this catalogue (Task 10), but a conversation
// already open when that shipped has its dynamic tool durably recorded under the OLD mount's own
// slug — `agent-kit__market_edge`, from the `defineDynamic` that used to live at
// `agent/extensions/agent-kit/tools/market_edge.ts` — not under this file's `"catalogue"` slug.
// This registration restores that old key's executor after a restart so those sessions keep
// replaying correctly until they end; see `durable-dynamic-tools.ts`'s own header and
// `agent/extensions/agent-kit/tools/market_edge.ts` for where the key used to be registered.
registerDurableDynamicTools("agent-kit__market_edge", { _single: market_edge });
