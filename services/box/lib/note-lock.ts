...
/**
 * Run `fn` while holding an advisory lock on ONE Brain note. Mirrors the
 * `note-lock.sh` flock pattern (the claude-obsidian pattern) but in-process,
 * for the box runtime's brain hand: writers to the SAME note serialise; writers
 ...
 * exclusive lockfile under `<brainRoot>/.locks/`.
 */
...
export function withNoteLock<T>(
  ...
  const lockDir = join(brainRoot, ".locks");
